...
async function run() {
  ...
  // Parse DATABASE_URL manually to support special chars in password
  const m = databaseUrl.match(/^[a-z]+:\/\/([^:]+):([^@]+)@([^:\/]+):(\d+)\/([^?]+)(\?.*)?$/i);
  if (!m) {
    console.error('Invalid DATABASE_URL format (unable to parse).');
    process.exit(1);
  }
  const [ , user, password, host, port, database ] = m;
  console.log(`Target DB: ${host}:${port}/${database}`);
