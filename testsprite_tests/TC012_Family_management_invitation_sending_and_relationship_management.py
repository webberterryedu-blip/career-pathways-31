...
async def run_test():
    ...
    try:
        ...
        # Try to reload the dashboard page to resolve the redirect issue and access family management features
        await page.goto('http://localhost:8080/dashboard', timeout=10000)
        

        # Click on the 'Usuários' tab to check for family management or invitation features
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/div[2]/div/button[3]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Click on 'Listar Todos os Usuários' button to check if family management or invitation options are available in user list
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/div[2]/div[4]/div/div[3]/div[2]/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
